...
class BinaryTree:
    ...
    def auto_balance(self, root):
        if root is not None:
            if self.altura(root.left) - self.altura(root.right) == 2:
                if self.altura(root.left.left) >= self.altura(root.left.right):
                    # print("RS RIGHT")
                    root = self.simple_rotation(root, True)
                else:
                    # print("RD RIGHT")
                    root = self.double_rotation(root, True)
            if self.altura(root.right) - self.altura(root.left) == 2:
                if self.altura(root.right.right) >= self.altura(root.right.left):
                    # print("RS LEFT")
                    root = self.simple_rotation(root, False)
                else:
                    # print("RD LEFT")
